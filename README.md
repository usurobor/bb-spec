...
Embodied Coherence is a protocol that issues non-transferable 🏷️ credentials for physical achievements verified by authorized 🛡️ Marshals under shared 🧾 Trials, with a required 🎥 Replay for every minted Badge.

### Key Features

- **Proof-of-Physical-Work (PoPW)** — Live-observed, 2-of-2 signed Records (👤 Contender + 🛡️ Marshal)
- **🏷️ Badges** — Non-transferable SBTs minted on ✅ PASS, linked to 🎥 Replay; do not expire (v1)
- **🎥 Replay** — Video evidence required for every ✅ PASS (off-chain storage, hash on-chain)
...
```
🧾 Select Trial → 🎥 Record → 👀 Live Run → ✍️ Co-sign → ⛓️ Submit → 🏷️ Badge + 🏆 Ladder
```

1. 👤 Contender selects 🧾 **(trialId, version)** and uses a tool matching the spec
2. 👤 Contender 🎥 **records the Run** (required for ✅ PASS / 🏷️ Badge)
3. 👤 Contender performs the Run under **live observation** (co-located or video)
4. 👤 Contender + 🛡️ Marshal co-sign one Record (2-of-2)
5. ⛓️ Record submitted on-chain
   - **✅ PASS**: mint 🏷️ Badge linked to 🎥 Replay
   - **⛔ NO PASS**: record Run; no Badge
6. 🏆 Ladders rank **verified ✅ PASS** per Trial (eligible versions only)
...
│   ├── pitch-deck.md      # Investor pitch (v0.8)
│   ├── executive-memo.md  # 1-page decision document (v0.3)
...
- 🎥 Replay required for ✅ PASS / 🏷️ Badge (prevents missing-evidence disputes)

---

## Documentation

- [Whitepaper](docs/whitepaper.md) — Full protocol specification (v1.0.17)
- [Technical Spec](docs/technical-spec.md) — Implementation details (v1.0.17)
- [Pitch Deck](docs/pitch-deck.md) — Investor pitch (v0.8)
- [Executive Memo](docs/executive-memo.md) — 1-page decision document (v0.3)
